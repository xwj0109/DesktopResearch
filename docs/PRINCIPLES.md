...
- **Registry coverage.** Ideas, sources, reviews, Research Development workspaces, data snapshots, feeds, runs, batches, risks, idea context and release candidates are covered (77 tools). The legacy records (search brief, bibliography, spec, contract, graph, code, conclusion) and their commands (handoffs, feasibility, datasets, reference-engine runs, exports) are still reachable only through the UI; they are being replaced rather than extended (docs/WORKFLOW-REDESIGN-PLAN.md).
